<form id="recalcShipping" method="POST" class="col-lg-6" action="{url action='calculateShippingCosts' sTargetAction=$sTargetAction}">
    ...
    {block name='frontend_checkout_shipping_costs_country'}
        <div class="form-group col-md-12 px-0">
            {block name='frontend_checkout_shipping_costs_country_label'}
                <label for="basket_country_list" class="control-label">{s name="ShippingLabelDeliveryCountry"}{/s}</label>
            {/block}
        ...
        {block name='frontend_checkout_shipping_costs_state'}
            {foreach $sCountryList as $country}
                {if $country.states}
                    <div class="form-group col-md-12 px-0{if $country.id != $sCountry.id} d-none{/if}">
                        {block name='frontend_checkout_shipping_costs_state_label'}
                            <label for="country_{$country.id}_states" class="control-label">{s name='RegisterBillingLabelState'}{/s}</label>
                        {/block}
    ...
    {block name='frontend_checkout_shipping_costs_payment'}
        <div class="form-group col-md-12 px-0">
            {block name='frontend_checkout_shipping_costs_payment_label'}
                <label for="basket_payment_list" class="control-label">{s name="ShippingLabelPayment"}{/s}</label>
            {/block}
    ...
    {block name='frontend_checkout_shipping_costs_dispatch'}
        <div class="form-group col-md-12 px-0">
            {block name='frontend_checkout_shipping_costs_dispatch_label'}
                <label for="basket_dispatch_list" class="control-label">{s name="ShipppingLabelDispatch"}{/s}</label>
            {/block}
