...
{block name='frontend_index_content'}
    <div class="account-change-payment mb-5" data-register="true">

        {* Payment headline *}
        {block name="frontend_account_payment_headline"}
            <h2 class="h3">{s name='PaymentHeadline'}{/s}</h2>
        {/block}
                ...
                <form name="frmRegister" method="post" action="{url action=savePayment sTarget=$sTarget}">
                    ...
                    {block name="frontend_account_payment_action_buttons"}
                        {block name="frontend_account_payment_action_button_send"}
                            <button type="submit" class="btn btn-primary">{s name='PaymentLinkSend'}{/s}</button>
                        {/block}
                        {block name="frontend_account_payment_action_button_back"}
                            {if $sTarget}
                                <a class="btn btn-outline-secondary" href="{url controller=$sTarget}" title="{s name='PaymentLinkBack'}{/s}">
