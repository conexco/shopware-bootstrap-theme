...
{block name="frontend_detail_config_step_form"}
    <form method="post" action="{url sArticle=$sArticle.articleID sCategory=$sArticle.categoryID}" class="config-step-form configurator-form sw5-plugin mb-3">
                ...
                {block name="frontend_detail_config_step_form_group"}
                        {block name="frontend_detail_config_step_form_group_inner"}
                            ...
                            {block name='frontend_detail_group_selection'}
                                <select{if $groupID gt 0 && empty($sArticle.sConfigurator[$pregroupID].user_selected)} disabled="disabled"{/if}
                                        name="group[{$sConfigurator.groupID}]"{if $theme.ajaxVariantSwitch}
                                        data-ajax-select-variants="true"{else} data-auto-submit="true"{/if}
                                        class="form-control">
