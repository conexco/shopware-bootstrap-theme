...
{block name='frontend_index_body'}
    <body
    {* Background Image *}
    {if $Controller || $BodyBgImg || $swVersion || $swrtVersion}
        class="{block name="frontend_index_body_classes"}ctl_{controllerName} is-act-{controllerAction}{if $sUserLoggedIn} is-user{/if}{if $sOneTimeAccount} is-one-time-account{/if}{if $BodyBgImg} bg-img-{$theme['body-bg-img-hide']}{/if} {if $swVersion && $swrtVersion}{$swVersion|replace:".":""}-{$swrtVersion|replace:".":""}{/if}{/block}"
    {/if}
    {if $BodyBgImg}
