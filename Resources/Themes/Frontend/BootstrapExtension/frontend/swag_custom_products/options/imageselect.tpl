                            ...
                            {block name="frontend_detail_swag_custom_products_options_imageselection_color"}
                                {if $value['image']}
                                    ...
                                    {if !empty($thumbnails)}
                                        ...
                                        {/foreach}
                                        <img class="color-selection-display-image img-responsive img-center" srcset="{$srcSet}" alt="{if $value['seo_title']}{$value['seo_title']|escapeHtml}{else}{$valueMedia['name']|escapeHtml}{/if}" itemprop="image" />
                                    {else}
                                        {$baseSource = $valueMedia['file']}
                                        <img class="color-selection-display-image img-responsive img-center" src="{$baseSource}" alt="{$valueMedia['name']|escapeHtml}" itemprop="image" />
                                    {/if}
                                {/if}
                            {/block}
