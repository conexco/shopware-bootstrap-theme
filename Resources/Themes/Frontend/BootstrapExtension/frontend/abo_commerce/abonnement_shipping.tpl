{block name="frontend_account_abonnement_shipping"}
    <tr>
        ...
        {block name="frontend_account_abonnement_shipping_values"}
            ...
            <a class="btn btn-default right mvm" href="{url controller=address action=edit id=$order.shipping.fallbackId orderId=$order.lastOrder.id}">{s name="ChangeShippingAddress" namespace="frontend/abo_commerce/index"}Change shipping address{/s}</a>
            <br>
        {/block}
    </tr>
{/block}